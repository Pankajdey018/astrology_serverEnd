...
async function getPanchanga(date, locationName = "Kolkata") {
  try {
    ...
    const karana = karanaIndex < 7
      ? Karanas[karanaIndex]
      : Karanas[7 + (tithiIndex % 4)];
    ...
    const abhijitStart = new Date(sunrise.getTime() + 6 * 60 * 60 * 1000 - 24 * 60 * 1000);
    ...
    return {
      ...
      moonrise: moonTimes.rise ? moonTimes.rise.toLocaleTimeString() : "No moonrise",
      moonset: moonTimes.set ? moonTimes.set.toLocaleTimeString() : "No moonset",
      tithi,
      karana,
      nakshatra,
      yoga,
